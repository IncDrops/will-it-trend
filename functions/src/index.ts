...
import {trendForecast} from '@/ai/flows/trend-forecasting'; // Ensure this is correctly implemented
import * as crypto from 'crypto';
import Stripe from 'stripe';
import {db, auth} from './firebase-admin'; // Ensure firebase-admin.ts is correctly configured
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // Stripe can return multiple line items; usually, for simple credit purchases, there's one.
      // Ensure you handle cases where line_items might be empty or structured differently.
...
const authenticateAndRateLimit = async (
  ...
  try {
    ...
    if (!apiKeyDoc.exists || !apiKeyDoc.data()?.active) {
      logger.warn(`Attempted use with invalid or inactive API key: ${apiKey}`);
    ...
    if (!tierLimit) {
      logger.error(`Invalid API tier '${tier}' for key: ${apiKey}`);
    ...
    const lastRequestDate = last_request_timestamp
      ? last_request_timestamp instanceof Date
        ? last_request_timestamp
        : last_request_timestamp.toDate()
      : null;

    // Determine if an hour has passed since the last request or if it's the very first request
    const isNewHour =
      !lastRequestDate || now.getTime() - lastRequestDate.getTime() > 3600 * 1000;
    ...
    if (currentRequestCount >= tierLimit) {
      logger.warn(`API Key ${apiKey} exceeded hourly limit for tier ${tier}.`);
    ...
    logger.info(
      `API Key ${apiKey} used. Request count: ${newCount}/${tierLimit}`
    );
    ...
    apiKeyDocRef
      ...
      .catch((logError) =>
        logger.error('Failed to log API key usage:', logError)
      );
    ...
    return res
      .status(500)
      .send({error: 'Internal server error during API key validation.'});
  }
};
...
// POST /v1/trend-forecast - Firebase Auth & Usage Check
app.post(
  '/v1/trend-forecast',
  authenticateFirebaseToken,
  ...
  async (req: Request, res: Response) => {
    ...
    if (!input || !timeHorizon) {
      logger.warn(
        `User ${uid}: Missing input or timeHorizon for trend forecast.`
      );
      return res
        .status(400)
    ...
    logger.info(
      `User ${uid}: Attempting trend forecast for input: "${input}", timeHorizon: "${timeHorizon}"`
    );
    ...
    try {
      ...
      return res.status(500).send({
        ...
        error:
          e.message ||
          'An unexpected error occurred during trend forecasting. Please try again.',
      });
    }
  }
);
...
app.get(
  ...
  (req: Request, res: Response) => {
    ...
    logger.info(
      `API Key user (tier: ${tier}) requested trends. Limiting to ${limit} results.`
    );
    return res.status(200).send({
      message: 'Trends endpoint hit successfully. (Placeholder data)',
      data: [
        {trend: '#SampleTrend', score: 95},
        {trend: '#AnotherTrend', score: 88},
        {trend: '#AIRevolution', score: 99},
        {trend: '#FutureTech', score: 92},
      ].slice(0, limit),
    });
  }
);

// POST /v1/predict - API Key Auth & Caching
app.post(
  '/v1/predict',
  authenticateAndRateLimit,
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (cachedDoc.exists) {
        ...
        // Ensure timestamp is a Firestore Timestamp object before calling toDate()
        const cacheTime =
...
// Using an array for middleware chain for better readability
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    logger.info(
      `User ${uid}: Generating captions for topic "${topic}" with tone "${tone}".`
    );
    try {
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!industry || !platform) {
      logger.warn(
        `User ${uid}: Missing industry or platform for best time to post.`
      );
      return res.status(400).send({
        error: 'Request body must include "industry" and "platform".',
      });
    }
    logger.info(
      `User ${uid}: Getting best time to post for industry "${industry}" on platform "${platform}".`
    );
    try {
